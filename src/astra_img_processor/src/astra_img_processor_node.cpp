	...
	img_tp_(nh_)
{
  ...
  image_pub_ = img_tp_.advertise("image_out", 100);
	...
	//set subscribers
	camera_info_subs_= nh_.subscribe("camera_info", 1, &AstraImgProcessorNode::cameraInfoCallback, this);
	image_info_subs_ = img_tp_.subscribe("image_in", 1, &AstraImgProcessorNode::imageCallback, this);
	depth_info_subs_ = img_tp_.subscribe("image_depth_in", 1, &AstraImgProcessorNode::depthCallback, this);
}
...
void AstraImgProcessorNode::cameraInfoCallback(const sensor_msgs::CameraInfoConstPtr& info_msg)
{
	//camera info
